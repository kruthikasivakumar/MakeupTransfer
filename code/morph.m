...
% Compute the correspondence field through morphing
  %im=imresize(im2double(imread(sprintf('../../data/%s/imgs/%s.png', style1, im1))), 0.5);
  %im = im2double(imread(sprintf('../../data/%s/imgs/%s.png', style1, im1)));
  im = im2double(imread('example_cr.png'));
  ...
  show_segs=1; show_before_after=1;
  if show_segs
  ...
  if show_before_after
    ...
    im_dst = im2double(imread('input_cr.png'));
    warped =warpImage(im, vx, vy);
...
function segs=load_face_model(style, im_name) 
  con = csvread('./face_spasm.con');
  %model = csvread(sprintf('../../data/%s/landmarks/%s.lm', style, im_name));
  %pwd
  %str_im = sprintf('../../data/%s/imgs/%s.png', style, im_name);
  str_im = im_name;
  %command_str = ['python write_landmarks_to_file.py shape_predictor.dat ' char(str_im)];
  command_str = ['sudo python write_landmarks_to_file.py ' char(str_im)];
  [status, c_out] = system(command_str);
  c_out
  if status ~= 0
